...
# For example, if there are 10 products and 3 products fit in a box, you need 4 boxes. 
# For instance, you could pack 3, 3, 2 and 2 products into the boxes.

# In a file boxes.py, implement the function min_count that returns the smallest number of boxes needed. 
...
# In each test case, both parameters are integers in the range 1–100.
...
def min_count(product_count, box_size):
  ...
  # uses plain floor division (where the ceiling and floor have been swapped), 
  # and then turns the world right-side up (with negation again)"
