...
# Thus the number of rounds is four.
# In a file fastrounds.py, implement the function count_rounds that takes the list as a parameter 
...
# In the last test case of the code template, 
# the list contains the numbers 1,2,...,10^5 in descending order and the number of rounds is 10^5. 
# Your function should be fast in this test case too.

def count_rounds(numbers):
    position = {num: index for index, num in enumerate(numbers)}
    
    rounds = 0
    n = len(numbers)
    
    for i in range(1, n+1):
        if position[i] < position.get(i - 1, -1):
            rounds += 1
